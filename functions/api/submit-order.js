...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!fullName || !phone || !wilaya) {
      console.error("❌ Validation échouée:", { fullName, phone, wilaya });
      return new Response(
    ...
    // 📧 DEBUG: Vérifier que l'API key existe
    ...
    if (env && env.RESEND_API_KEY) {
      ...
      try {
        ...
        console.error("❌ Erreur fetch Resend:", err.message);
      }
    } else {
      console.log("⚠️ RESEND_API_KEY non configurée dans Cloudflare");
    }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        orderId,
        message: "Commande Kiffane enregistrée",
        order: orderRecord,
      }),
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="section">
        ...
        <div class="info-row"><span class="info-label">Nom :</span><span class="info-value">${customer.fullName}</span></div>
        <div class="info-row"><span class="info-label">Téléphone :</span><span class="info-value">${customer.phone}</span></div>
        <div class="info-row"><span class="info-label">Wilaya :</span><span class="info-value">${customer.wilaya}</span></div>
        <div class="info-row"><span class="info-label">Livraison :</span><span class="info-value">${customer.deliveryType === "desk" ? "Stop Desk Yalidine" : "À Domicile"}</span></div>
      </div>
      <div class="section">
        ...
        <div class="product-box">
          ...
          <div class="info-row"><span>Prix :</span><span>${cart.productPrice.toLocaleString("fr-FR")} DA</span></div>
        </div>
        ${cart.includeBump ? `<div class="product-box"><div class="product-name">👛 Portefeuille Bonus</div><div class="info-row"><span>Prix :</span><span>${cart.bumpPrice.toLocaleString("fr-FR")} DA</span></div></div>` : ''}
      </div>
      <div class="section">
        <div class="totals">
          <div class="total-row"><span>Sous-total :</span><span>${totals.subtotal.toLocaleString("fr-FR")} DA</span></div>
          ${totals.bump > 0 ? `<div class="total-row"><span>Bonus :</span><span>+${totals.bump.toLocaleString("fr-FR")} DA</span></div>` : ''}
    ...
    <div class="footer">
      <p>Email généré par Kiffane.com</p>
    </div>
  </div>
</body>
</html>`;
